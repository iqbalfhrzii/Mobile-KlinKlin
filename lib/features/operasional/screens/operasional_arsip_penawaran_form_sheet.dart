...
class OperasionalArsipPenawaranFormSheet extends StatefulWidget {
  final dynamic initialData;
  final List<dynamic> cabangList;
  final VoidCallback onSave;

  const OperasionalArsipPenawaranFormSheet({
    super.key,
    this.initialData,
  ...
  @override
  State<OperasionalArsipPenawaranFormSheet> createState() => _OperasionalArsipPenawaranFormSheetState();
}

class _OperasionalArsipPenawaranFormSheetState extends State<OperasionalArsipPenawaranFormSheet> {
  ...
  String? _existingFilePath; // To show existing file name if editing
  ...
  void _initData() {
    if (widget.initialData != null) {
      ...
      _existingFilePath = data['file_path'];
    } else {
  ...
  Future<void> _save() async {
    if (_nomorController.text.isEmpty || _tanggalController.text.isEmpty || _selectedCabangId == null || _perihalController.text.isEmpty) {
    ...
    final data = {
      ...
      'nominal': _nominalController.text.isEmpty ? 0 : _nominalController.text,
      'keterangan': _keteranganController.text,
    };

    Map<String, dynamic> res;
    if (widget.initialData != null) {
      res = await _service.updateArsip(widget.initialData['id'], data, filePath: _selectedFilePath);
    } else {
  ...
  Widget _buildTextField(String label, TextEditingController controller, {bool required = false, bool readOnly = false, VoidCallback? onTap, int maxLines = 1, TextInputType? keyboardType, String? hint}) {
    return Column(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      widget.initialData != null ? 'Edit Arsip Nomor & Nominal' : 'Tambah Arsip Nomor & Nominal',
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('Nomor', _nomorController, required: true, hint: 'Nomor surat/penawaran'),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: _buildTextField('Tanggal', _tanggalController, required: true, readOnly: true, onTap: _selectDate),
                      ),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 16),
                      Expanded(
                        child: _buildTextField('Perihal', _perihalController, required: true, hint: 'Cth: Penawaran Cleaning'),
                      ),
                    ],
                  ),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('Nama Customer', _namaCustomerController, hint: 'Nama Customer'),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: _buildTextField('Nominal', _nominalController, keyboardType: TextInputType.number, hint: 'Rp 0'),
                      ),
                    ],
                  ),
                  const SizedBox(height: 16),
                  _buildTextField('Keterangan', _keteranganController, maxLines: 3, hint: 'Keterangan tambahan...'),
                  
                  const SizedBox(height: 16),
                  Text('File Bukti/Dokumen', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                  const SizedBox(height: 8),
                  Container(
                    decoration: BoxDecoration(
                      color: Colors.white,
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  child: _isLoading
                      ...
                      : Text('Simpan Arsip', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: Colors.white)),
                ),
              ],
            ),
          ),
