...
class OperasionalSimFormSheet extends StatefulWidget {
  final dynamic initialData;
  final List<dynamic> cabangList;
  final VoidCallback onSave;

  const OperasionalSimFormSheet({
    super.key,
    this.initialData,
  ...
  @override
  State<OperasionalSimFormSheet> createState() => _OperasionalSimFormSheetState();
}

class _OperasionalSimFormSheetState extends State<OperasionalSimFormSheet> {
  ...
  void _initData() {
    if (widget.initialData != null) {
      ...
      _existingFilePath = data['foto_sim'];
  ...
  Future<void> _save() async {
    if (_selectedCabangId == null || _namaKaryawanController.text.isEmpty || 
        _selectedJenisSim == null || _nomorSimController.text.isEmpty || _masaBerlakuController.text.isEmpty) {
    ...
    final data = {
      ...
      'nomor_sim': _nomorSimController.text,
      'tanggal_terbit': _tanggalTerbitController.text,
      'masa_berlaku': _masaBerlakuController.text,
      'kendaraan_dioperasikan': _kendaraanController.text,
      'keterangan': _keteranganController.text,
    };

    Map<String, dynamic> res;
    if (widget.initialData != null) {
      res = await _service.updateSimData(widget.initialData['id'], data, filePath: _selectedFilePath);
    } else {
      res = await _service.storeSimData(data, filePath: _selectedFilePath);
    }
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      widget.initialData != null ? 'Edit Data SIM' : 'Tambah Data SIM',
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('Pemilik SIM', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Pilih Karyawan', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                            const SizedBox(height: 8),
                            Container(
                              height: 48,
                              padding: const EdgeInsets.symmetric(horizontal: 16),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField('Nama Pemilik', _namaKaryawanController, required: true, hint: 'Nama Karyawan...'),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Icon(Icons.badge_outlined, size: 16, color: _indigoColor),
                            const SizedBox(width: 8),
                            Text('Data SIM', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.bold, color: _indigoColor)),
                          ],
                        ),
                        const SizedBox(height: 16),
                        ...
                        Row(
                          children: [
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonHideUnderline(
                                      child: DropdownButton<String>(
                                        ...
                                        items: const [
                                          DropdownMenuItem(value: 'SIM A', child: Text('SIM A')),
                                          DropdownMenuItem(value: 'SIM A Umum', child: Text('SIM A Umum')),
                                          DropdownMenuItem(value: 'SIM B I', child: Text('SIM B I')),
                            ...
                            const SizedBox(width: 16),
                            Expanded(
                              child: _buildTextField('Nomor SIM', _nomorSimController, required: true, keyboardType: TextInputType.number, hint: 'Nomor SIM...'),
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: _buildTextField('Tanggal Terbit', _tanggalTerbitController, readOnly: true, onTap: () => _selectDate(_tanggalTerbitController), hint: 'yyyy-mm-dd'),
                            ),
                            const SizedBox(width: 16),
                            Expanded(
                              child: _buildTextField('Masa Berlaku', _masaBerlakuController, required: true, readOnly: true, onTap: () => _selectDate(_masaBerlakuController), hint: 'yyyy-mm-dd'),
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
                        
                        _buildTextField('Kendaraan Dioperasikan', _kendaraanController, hint: 'Mobil Operasional Innova, Truk Engkel, Motor Matic...'),
                        
                        const SizedBox(height: 16),
                        _buildTextField('Keterangan Tambahan', _keteranganController, maxLines: 2, hint: 'Mis. Proses perpanjangan, hilang, dll...'),
                      ],
                    ),
                  ),
                  
                  const SizedBox(height: 24),
                  Text('Foto SIM', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                  const SizedBox(height: 4),
                  Text('Unggah foto SIM bagian depan.', style: GoogleFonts.inter(fontSize: 10, color: Colors.grey)),
                  const SizedBox(height: 8),
                  Container(
                    decoration: BoxDecoration(
                      color: Colors.white,
