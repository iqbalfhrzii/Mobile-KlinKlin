...
class FinanceCancelDetailScreen extends StatefulWidget {
  const FinanceCancelDetailScreen({super.key, required this.order});
  final OrderModel order;

  @override
  State<FinanceCancelDetailScreen> createState() => _FinanceCancelDetailScreenState();
}

class _FinanceCancelDetailScreenState extends State<FinanceCancelDetailScreen> {
  ...
  Future<void> _handleApproveCancel() async {
    ...
    try {
      if (order.pembatalanId == null) {
        throw Exception('ID Pembatalan tidak ditemukan');
      }
      await _financeService.approvePembatalan(order.pembatalanId!);
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Pembatalan berhasil disetujui'),
                ...
                children: [
                  ...
                  if (order.pembayaran != null) ...[
                    _buildSectionCard('INFORMASI PEMBAYARAN', [
                      _buildInfoRow(Icons.credit_card_outlined, 'Metode Bayar', order.pembayaran!.metodePembayaran, isBold: true),
                      const SizedBox(height: 16),
                      _buildInfoRow(Icons.access_time, 'Status Pembayaran', order.pembayaran!.statusPembayaran, isBold: true),
                      const SizedBox(height: 16),
                  ...
                  // Alasan Pembatalan Card
                  _buildSectionCard('ALASAN PEMBATALAN', [
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                  ...
                  // Bukti Pembatalan Card (if any)
                  if (order.cancelProof != null && order.cancelProof!.isNotEmpty)
                    _buildSectionCard('BUKTI PEMBATALAN', [
                      GestureDetector(
                        onTap: () => _showImageDialog(context, order.cancelProof!),
                        child: Container(
                    ...
                    if (_isProcessing)
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _handleApproveCancel,
