...
| `GET` | `/cleaner/jobs/history` | Ambil riwayat job selesai milik cleaner login |
...
## 2. History Job Cleaner Login

Endpoint ini khusus halaman history cleaner. Tidak memakai `cleaner_id`; cleaner diambil dari token login.

### Request

```http
GET /api/cleaner/jobs/history
...
- wajib login memakai token Sanctum
- hanya menampilkan job milik cleaner yang sedang login
- hanya menampilkan job dengan `status_pengerjaan = finished`
- `data.total_bonus` adalah total semua bonus dari job selesai milik cleaner login
- `data.pesanans` berisi pesanan yang sudah diselesaikan cleaner login
- setiap item di `data.pesanans` tetap punya `total_bonus` per job/pesanan cleaner
- detail pesanan tampil di `data.pesanans.*.pesanan.details`
...
{
  "status": true,
  "message": "Riwayat job cleaner berhasil diambil",
  "data": {
    "total_bonus": 70000,
    "pesanans": [
      {
        ...
        "notified_at": "2026-06-16T08:30:00.000000Z",
        "started_at": "2026-06-16T09:00:00.000000Z",
        "finished_at": "2026-06-16T11:00:00.000000Z",
        "total_bonus": 35000,
        "bonuses": [
          {
            "id": 1,
        ...
        "pesanan": {
          ...
          "tanggal_input": "2026-06-16T07:00:00.000000Z",
          "keterangan_order": "Cuci sofa ruang tamu",
          "subtotal": 300000,
          "pelanggan": {
            "id": 5,
            "nama_pelanggan": "Ibu Sari"
